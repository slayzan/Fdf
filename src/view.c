...
/*   Updated: 2019/06/05 09:22:52 by humarque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	isometric1(t_mlx *graph,int x, int y,t_build *param, int i)
{
	int prev_x;
	int prev_y;
	int savex;
	...
	if (prev_y >= 30)
	{
			...
			savey = prev_y - 30;
			bresenham(x, y, ft_calx(prev_x,savey,param), ft_caly(prev_x,savey,i,param), graph);
